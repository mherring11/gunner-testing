...
test('Check "Full Test No Awaits', async ({ browser }) => {
  ...
  await page.fill("#mui-1", "gunnerplaywright+0312b@gmail.com");
  ...
  const iframeSelector =
    'iframe[src*="https://visualizer.gunnerroofing.com/51892087-c6db-4d4e-b56e-e07cf8e1b3c9/"]';
  ...
  initialTotalEstimate = parseFloat(totalEstimateTexts.replace(/[^0-9.]/g, ""));
  ...
  console.log(
    `Address displayed on Statement of Work page: '${addressDisplayed}'.`.yellow
  );
  ...
  console.log(
    "Verified: All selected add-ons sections are displayed on the page.".yellow
  );
  ...
  await page.waitForFunction(() => {
    ...
    return (
      totalPriceElement && !totalPriceElement.textContent.includes("Loading...")
    );
  });
  ...
  if (depositAmount && Math.abs(depositAmount - expectedDepositAmount) < 0.01) {
    console.log(
